...
function TOOL:DoParent(Ent1, Ent2)
    ...
    if not (Ent1 and Ent1:IsValid() and Ent1:EntIndex() ~= 0) then
        self:SendMessage("Oops, First Target was world or something invalid")

        return
    end

    if not (Ent2 and Ent2:IsValid() and Ent2:EntIndex() ~= 0) then
    ...
    while true do
        if (not TempEnt:GetParent():IsValid()) then
            Ent1:SetParent(Ent2)

            if self:GetClientNumber("entirecontrap") == 0 then
                self:SendMessage("Parent Set.")
            end
...
function TOOL:DoApply(CurrentEnt, FirstEnt, autorotate, nocollideall, ShadowDisable)
    ...
    if (autorotate) then
        --Snap-rotate original object first.  Rest needs to rotate around it.
    ...
    CurrentEnt:DrawShadow(not ShadowDisable)
    ...
    for key, CurrentEnt in pairs(self.TaggedEnts) do
        if (CurrentEnt and CurrentEnt:IsValid()) then
            if not (CurrentEnt == Ent2) then
                ...
                --parent?
                if (CurrentPhys:IsValid() and not CurrentEnt:GetParent():IsValid()) then
                    --not a ragdoll
                    if (CurrentEnt:GetPhysicsObjectCount() < 2) then
                        ...
                        if (speeddamp == 0 and angledamp == 0) then
                            CurrentPhys:SetDamping(5, 5)
                        elseif (speeddamp == 5 and angledamp == 5) then
        ...
        if (CurrentEnt and CurrentEnt:IsValid()) then
            if not (CurrentEnt == Ent2) then
                ...
                if (CurrentPhys:IsValid() and not CurrentEnt:GetParent():IsValid()) then
                    --not a ragdoll
                    if (CurrentEnt:GetPhysicsObjectCount() < 2) then
                        -- not weld/axis/ballsocket or single application
                        ...
                        elseif (mode == 10 and not self.RepairTodo) then
                            --Repair spaz
                            if (CurrentEnt:GetCollisionGroup() == COLLISION_GROUP_WORLD) then
                                CurrentEnt:SetCollisionGroup(COLLISION_GROUP_NONE)
                            elseif (CurrentEnt:GetCollisionGroup() == COLLISION_GROUP_NONE) then
                        ...
                        if (mode <= 8) then
                            CurrentPhys:EnableMotion(not freeze)
                            CurrentPhys:Wake()
                        end
                    end
                end
...
--TODO: Parented stuff should return 1
...
function TOOL:DoMove()
    ...
    --Set axies for rotation mode directions
...
function TOOL:LeftClick(trace)
    ...
    --first click - choose a target.
    if (stage == 0) then
        if (self:TargetValidity(trace, Phys) <= 1) then return false end --No phys or hit world
        ...
        --Don't care about stage, always apply.
            ...
            for key, ConstraintEntity in pairs(Ent.Constraints) do
                if (not ConstsTab[ConstraintEntity]) then
                    ...
                    for i = 1, 6 do
                        ...
                        if (e and e:IsValid()) and (not EntsTab[e:EntIndex()]) then
        ...
        for key, Ent in pairs(EntsTab) do
            if (Ent and Ent:IsValid()) then
                ...
                for key, Constraint in pairs(MyTable) do
                    if (not ConstsTab[Constraint.Constraint]) then
...
function TOOL:Think()
    ...
    if not wep:IsValid() or wep:GetClass() ~= "gmod_tool" or GetConVar("gmod_toolmode"):GetString() ~= "precision" then return end
    ...
    if self:NumObjects() == 1 and mode ~= 2 then
        if ((self:GetClientNumber("move") == 1 and mode >= 3) or mode == 3) then
            --no move = no ghost in parent mode
            if (mode <= 8) then
...
if CLIENT then
    ...
    local function AddDefControls(Panel)
        ...
        Panel:AddControl("Checkbox", {
            Label = "Push/Pull as Percent (%) of target's depth",
        ...
        if (mode >= 4 and mode <= 8) then
            Panel:AddControl("Checkbox", {
                Label = "Move Target? ('Easy' constraint mode)",
        ...
        if (mode >= 3 and mode <= 8) then
            Panel:AddControl("Checkbox", {
                Label = "Rotate Target? (Rotation after moving)",
            ...
            Panel:AddControl("Checkbox", {
                Label = "Snap distance as Percent (%) of target's depth",
        ...
        if (mode <= 8) then
            Panel:AddControl("Checkbox", {
                Label = "Freeze Target",
                Command = "precision_freeze",
                Description = "Freeze props when this tool is used"
            })

            if (mode >= 3 and mode <= 8) then
                Panel:AddControl("Checkbox", {
                    Label = "No Collide Targets",
                    Command = "precision_nocollide",
                    Description = "Nocollide pairs of props when this tool is used. Note: No current way to remove this constraint when used alone."
                })
            end
        end
        ...
        if (user >= 2 or mode == 1) then
            if ((mode >= 3 and mode <= 8) or mode == 1) then
                Panel:AddControl("Checkbox", {
                    Label = "Auto-align to world (nearest 45 degrees)",
            ...
            if (mode == 1) then
                Panel:AddControl("Checkbox", {
                    Label = "Disable target shadow",
                    Command = "precision_ShadowDisable",
                    Description = "Disables shadows cast from the prop"
                })
            end
        end
        ...
        if (user >= 3) then
            ...
            if (mode == 1) then
                ...
                Panel:AddControl("Checkbox", {
                    ...
                    Command = "precision_physdisable",
                    Description = "Disables physics on the first prop (gravity, being shot etc won't effect it)"
                })

                Panel:AddControl("Checkbox", {
                    Label = "Adv: Allow Physgun on PhysDisable objects",
                    Command = "precision_allowphysgun",
                    Description = "Disabled to stop accidents, use if you want to be able to manually move props after phyics disabling them (may break clipboxes)."
                })
            ...
            if (mode == 9) then
                Panel:AddControl("Checkbox", {
                    Label = "Adv: Allow Physgun on Parented objects",
                    Command = "precision_allowphysgun",
                    Description = "Disabled to stop accidents, use this if you want to play with the parenting hierarchy etc."
                })
            end
        end

        if (user >= 2) then
            --todo: entire contrap move/rotate support
            if (mode ~= 2 and mode ~= 3 and mode ~= 10) then
                Panel:AddControl("Checkbox", {
                    Label = "Entire Contraption! (Everything connected to target)",
                    Command = "precision_entirecontrap",
                    Description = "For mass constraining or removal or nudging or applying of things. Yay generic."
                })
            end
        end
        ...
        if (mode == 7) then
            ...
            Panel:AddControl("Checkbox", {
                Label = "Free Movement",
                Command = "precision_FreeMov",
                Description = "Only lock relative rotation, not position?"
            })
        end
        ...
        if (mode == 8) then
            ...
            Panel:AddControl("Checkbox", {
                ...
                Command = "precision_disablesliderfix",
                Description = "Fix being separate X/Y/Z advanced ballsocket locks between the props.  This stops most spaz caused by rotation, but not spaz caused by displacement."
            })

            Panel:AddControl("Label", {
                Text = "Stabilisation is separate X/Y/Z adv. ballsockets; it makes it far less prone to rotation triggered spaz, but the difference is only noticeable sometimes as it's still just as prone to spaz caused by drifting.",
                Description = "Due to lack of working descriptions at time of coding"
            })
        end
        ...
        if (mode == 9) then
            Panel:AddControl("Label", {
                Text = "Parenting Notes:",
                Description = "Due to lack of working descriptions at time of coding"
            })

            Panel:AddControl("Label", {
                Text = "Parenting objects is most similar to a very strong weld, but it stops most interaction on the first object when you attach it to the second.  Players can walk on it, but it will fall through players.  It will not collide with objects or the world.  It will also not cause any extra physics lag/spaz.  Try it out on a test object, and decide if it's useful to you!",
                Description = "Due to lack of working descriptions at time of coding"
            })

            Panel:AddControl("Label", {
                Text = "Parented objects are most useful for: Adding detail to moving objects without creating extra physics lag.  Things like houses that you want to move (though you can only safely walk on parented objects when they are still.)",
                Description = "Due to lack of working descriptions at time of coding"
            })

            Panel:AddControl("Label", {
                Text = "Possible issues:  Remove constraints first to avoid spaz. Duplicating or such may cause the collision model to become separated.  Best to test it if in doubt.",
                Description = "Why must labels cause menu flicker? D:"
            })
        end
        ...
        if (mode == 10) then
            ...
            Panel:AddControl("Label", {
                Text = "Usage: When a contraption is going crazy, colliding, making rubbing noises.",
                Description = ""
            })

            Panel:AddControl("Label", {
                Text = "What it does: Temporarily toggles collisions, allowing things that are bent out of shape to pop back.",
                Description = ""
            })

            Panel:AddControl("Label", {
                Text = "Warning: No guarantees.  This may turn things inside-out or make things worse depending on the situation.",
                Description = ""
            })
        end
        ...
        if (mode == 11) then
            Panel:AddControl("Label", {
                Text = "This mode will remove:",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Nocollide",
                Command = "precision_removal_nocollide",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Weld",
                Command = "precision_removal_weld",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Axis",
                Command = "precision_removal_axis",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Ballsocket",
                Command = "precision_removal_ballsocket",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Adv. Ballsocket",
                Command = "precision_removal_advballsocket",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Slider",
                Command = "precision_removal_slider",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Parent",
                Command = "precision_removal_parent",
                Description = ""
            })

            Panel:AddControl("Checkbox", {
                Label = "Other",
                Command = "precision_removal_other",
                Description = ""
            })

            Panel:AddControl("Label", {
                Text = "(Other = Rope/slider variants like winch/hydraulic, also motor/keepupright)",
                Description = ""
            })

            Panel:AddControl("Button", {
                Label = "Select All",
                Command = "precision_removal_all",
                Description = ""
            })

            Panel:AddControl("Button", {
                Label = "Select None",
                Command = "precision_removal_none",
                Description = ""
            })
        end
        ...
        if (showgenmenu == 1) then
            Panel:AddControl("Button", {
                Label = "\\/ General Tool Options \\/",
                Command = "precision_generalmenu",
                Description = "Collapse menu"
            })

            local params = {
                Label = "User Level",
                Description = "Shows options appropriate to user experience level",
                MenuButton = "0",
                Height = 67,
                Options = {}
            }
            ...
            Panel:AddControl("Checkbox", {
                Label = "Enable tool feedback messages?",
                Command = "precision_enablefeedback",
                Description = "Toggle for feedback messages incase they get annoying"
            })

            Panel:AddControl("Checkbox", {
                Label = "On = Feedback in Chat, Off = Centr Scrn",
                Command = "precision_chatfeedback",
                Description = "Chat too cluttered? Can have messages centre screen instead"
            })
            ...
            Panel:AddControl("Checkbox", {
                Label = "Add Push/Pull to Undo List",
                Command = "precision_nudgeundo",
                Description = "For if you're in danger of nudging somthing to where you can't reach it"
            })

            Panel:AddControl("Checkbox", {
                Label = "Add Movement to Undo List",
                Command = "precision_moveundo",
                Description = "So you don't have to secondary fire with nocollide to undo mistakes"
            })

            Panel:AddControl("Checkbox", {
                Label = "Add Rotation to Undo List",
                Command = "precision_rotateundo",
                Description = "So you can find the exact rotation value easier"
            })

            Panel:AddControl("Button", {
                Label = "Restore Current Mode Default",
                Command = "precision_defaultrestore",
                Description = "Collapse menu"
            })
        else
            Panel:AddControl("Button", {
            ...
            if (user == 1) then
                Panel:AddControl("Label", {
                    Text = "(Note: For more modes and options like slider, use this options button and change the user level)",
                    Description = ""
                })
            end
        end
    ...
    function precision_setuser(player, tool, args)
        if GetConVar("precision_user", 3):GetInt() ~= args[1] then
    ...
    function TOOL:FreezeMovement()
        ...
        if (stage == 2) then return true end --elseif ( iNum > 0 && self:GetClientNumber("mode") == 2 ) then --	return true
